...

{
  model_df <- main_df %>% 
    ...
    select(
      ...
      Signal,
      RSI,
      #RETURN_1Y, RETURN_6M, RETURN_1M, RETURN_1W,
      RETURN_3M, 
...
corr_df <- model_df %>% 
  select(
    ...
    financial_leverage, annualized_rolling_volatility, sentiment_1lag, RSI, RETURN_3M, MACD_Indication,
...
long_model_df <- model_df %>%
  tidyr::pivot_longer(cols = c(RETURN_3M, MACD_Indication, RSI),
               names_to = "variable", values_to = "value")

variable_names <- list(
  'MACD_Indication'="MACD",
  'RETURN_3M'="RETURN_3M",
  'RSI'="RSI"
)
...
ggplot(long_model_df, aes(x = value, y = sentiment)) +
  geom_point() +
...
# Trend Models
sentiment_formFull <- sentiment ~ RETURN_3M + MACD_Indication + RSI
...
stargazer::stargazer(full.models$full,
                     ...
                     column.labels = c("Full Model", "Return 3M", "MACD", "RSI", "Reference Model", "Full Model (TR)"))
...
stargazer::stargazer(sent.models[[1]],
                     ...
                     #type = "text",
                     keep.stat = c("n", "rsq", "adj.rsq", "aic", "bic"), 
                     column.labels = c("Full Model", "Return 3M", "MACD", "RSI"),
                     title = "Regression of Sentiment against Market Trend Indicators")

# Plotting residuals vs fitted
full.models.plot <- list(
  "Full Model (1)" = full.models$full,
  "Return 3M (2)" = full.models$r3m,
  "MACD (3)" = full.models$MACD,
  "RSI (4)" = full.models$RSI,
  trf = full.models$trf
)
...
ggplot(plot_data, aes(x = Fitted, y = Residuals)) +
  ...
  labs(x = "Fitted Values", y = "Residuals", 
       #title = "Residuals vs Fitted Values for Multiple Models"
       )
...
train_control_ols <- trainControl(method = "LOOCV", savePredictions = "all", returnResamp = "all")
...
model_names <- c("ctrl", "full", "r3m", "MACD", "RSI")
...
# Existing code for LOOCV with Caret and OLS
train_control_ols <- trainControl(method = "LOOCV", savePredictions = "all", returnResamp = "all")
loocv.models <- map(forms_control, ~train(.x, data = model_df, method = "lm", trControl = train_control_ols))
...
test_rmse_df <- lapply(model_names, function(model) {
  loocv.models[[model]]$results$RMSE
}) %>% data.frame()
names(test_rmse_df) <- model_names
test_rmse_df <- test_rmse_df %>% mutate(metric = "testRMSE", model = "OLS")

# Calculate Training RMSEs
train_rmse_df <- lapply(model_names, function(model) {
  predictions <- loocv.models[[model]]$pred
  sqrt(mean((predictions$pred - predictions$obs)^2))
}) %>% data.frame()
names(train_rmse_df) <- model_names
train_rmse_df <- train_rmse_df %>% mutate(metric = "trainRMSE", model = "OLS")

# Combine Training and Testing RMSEs
combined_rmse_df <- rbind(test_rmse_df, train_rmse_df)

# View the combined RMSE dataframe
...
# Variable Importance
importance.full <- varImp(xgb_tuned)
importance.full
...

{
  ...
  saveRDS(xbg.models, file = "../r-data/xgb_models.rds")
  xbg.models <- readRDS(file = "../r-data/xgb_models.rds")
  model_names <- c("ctrl","full", "r3m", "MACD", "RSI")
  rmse_df <- lapply(model_names, function(model) {
    # Replace loocv.models with your actual list variable
...
RMSE.df <- rbind(RMSE.df.ols, RMSE.df.xgb) %>% 
  rename("Reference" = "ctrl",
         "Full Model" = "full",
         "Return 3M" = "r3m",
         "Model" = "model",
         "Metric" = "metric") %>% 
  select(Metric, Model, Reference, 'Full Model', 'Return 3M', 'MACD', 'RSI')

xtable(RMSE.df, digits=c(0,0,0,5,5,5,5,5))


# Variable Importance
importance.control <- varImp(xbg.models$ctrl)
importance.full
importance.control
...
importance.control_df$Source <- "Reference Model"

# Combine the data frames
combined_df <- rbind(importance.full_df, importance.control_df) %>% 
  rename(Importance = "Overall")

missing_variables <- setdiff(combined_df[combined_df$Source == "Full Model", "Variable"], combined_df[combined_df$Source == "Reference Model", "Variable"])
bind_rows(
  combined_df,
  data.frame(
    Variable = missing_variables,
    Importance = rep(0, length(missing_variables)),
    Source = rep("Reference Model", length(missing_variables))
  )
) -> combined_df_plot
...
gam=gam(sentiment~analyst+log(market_cap)+log(time_on_OSE)+s(pe_ratio)+financial_leverage+annualized_rolling_volatility+
          sentiment_1lag + RSI + RETURN_3M + MACD_Indication + VIX,data=train_df)
par(mfrow =c(4,3))
