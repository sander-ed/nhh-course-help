...
set U;		# Week numbers in the period
set V {U};	# Days in week number U
...
var f {D} binary; 		# If working from home at day D
...
subject to
	...
	#For all workdays, must be driving to and from work except when working from home
	DriveToWork {d in W}: y[d,7] = 1-f[d];
	DriveFromWork {d in W}: y[d,16] = 1-f[d];
	
	#For all workdays, cannot charge while at the office but can at the home office
	CantChargeAtWork {d in W, h in 8..16}: x[d,h] <= f[d]*r;
	...
	StartHours {d in G}: sum {h in {10..13}} s[d,h] = 1;
	...
	#For all weeks, must work from home one day
	OneDayWFH {u in U}: sum{d in V[u]} f[d] = 1;
	
	#For the days of the partial week at the start of the period, optidriver cannot work from home 
	Drive1stDay: y[1,7] = 1;
	Drive2stDay: y[1,16] = 1;
