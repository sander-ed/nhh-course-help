...
#----- VARIABLES -----
var x{i in Teams,j in Teams,k in Days:i<>j} binary; # 1 if team i plays a home game against team j on day k, zero otherwise
var w{Weekends} binary; #1 if a Super Week is completed on Weekend day k
var y{i in Teams, d in Tuesdays:i<>"NTNU"} binary;# 1 if team i plays on tuesday day d and weekend day d+1

#----- OBJECTIVE FUNCTION -----
maximize fobj:
	sum{d in Weekends}w[d];

subject to
FOToughWeeks:
	sum{i in Teams, d in Tuesdays: i<>"NTNU"}y[i,d]=6;
...
#var w{Weekends} binary; #1 if a Super Week is completed on Weekend day k: superweek means at least 2 matches on Tue and NTNU plays two matches at home on the weekend
Logical_wx{d in Weekends}:
2*w[d] <= sum{i in Teams, j in Teams:i<>j}x[i,j,d-1];

Logical_wx2{d in Weekends}:
2*w[d] <= sum{j in Teams: j<>'NTNU'}(x['NTNU',j,d] + x[j,'NTNU',d]);

#var y{i in Teams, d in Tuesdays:i<>"NTNU"} binary;# 1 if team i plays on tuesday day d and weekend day d+1
Logical_yx{i in Teams, d in Tuesdays:i<>"NTNU"}:
	sum{j in Teams: i<>j}(x[i,j,d] + x[i,j,d+1] + x[j,i,d] + x[j,i,d+1]) <= 1 + y[i,d];

Logical_yx2{i in Teams, d in Tuesdays:i<>"NTNU"}:
	y[i,d] <= sum{j in Teams: i<>j}(x[i,j,d] + x[j,i,d]);

Logical_yx3{i in Teams, d in Tuesdays:i<>"NTNU"}:
	y[i,d] <= sum{j in Teams: i<>j}(x[i,j,d+1] + x[j,i,d+1]);
