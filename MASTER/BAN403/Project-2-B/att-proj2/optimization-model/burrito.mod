...
#param a{I,J}; # demand multiplier for customer i and truck j. This multiplier depends on the distance: the further a customer i is from a truck location j, the less willing the customer is to walk to the truck
...
maximize profit:
	sum{i in I, j in J} (r-k) * y[i,j] * d[i] * a[i,j] - 
	sum{j in J} (f * x[j])
;
